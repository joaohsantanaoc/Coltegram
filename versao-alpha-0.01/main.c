...
int main(int argc, char **argv) {
    ...
    perfil_t *ponteiro_perfil = NULL; // tem todos os dados
    login_t login_info;
    int num_perfis = 0;
    bool logado = false;
    ...
    arquivoDados = fopen("arquivoDados.txt", "r");

    if (arquivoDados == NULL){
            printf("Erro ao ler os dados!");
        }

    while (fgets(manipulacaoDeDados, Tamanho_Gigante, arquivoDados) != NULL)
    {
        num_perfis++;
    }
    ...
    for (int i = 0; i < num_perfis; i++){
        ...
        if (contadorDeBarra == 1){

            while (fgets(ponteiro_perfil[i].nome_usuario, NUM_MAX_CARACTERES_NOME_USUARIO, arquivoDados) != ponteiro_perfil[i].ID && '|');

            contadorDeBarra ++;
        }
        
        printf("%s\n%s", ponteiro_perfil[i].ID, ponteiro_perfil[i].nome_usuario);
    }
